"""Forecasting metrics: accuracy, calibration (ECE), rare-state recall, summaries."""
...
def expected_calibration_error(
    ...
    Bins predictions by their confidence and measures the gap between mean
    confidence and empirical accuracy in each bin, weighted by bin population.
    A well-calibrated forecaster (confidence matches correctness) has ECE -> 0.
    """
    y_true = np.asarray(y_true)
    y_pred = np.asarray(y_pred)
...
def rare_state_recall(y_true: np.ndarray, y_pred: np.ndarray, rare_state: int) -> float:
    """Recall on the rare state in [0, 1]: of the true rare cells, how many we find.

    Returns 1.0 by convention when the test set contains no rare cells (nothing
    to miss), so it never spuriously penalizes a split.
    """
    y_true = np.asarray(y_true)
    y_pred = np.asarray(y_pred)
    pos = y_true == rare_state
    n_pos = int(pos.sum())
    if n_pos == 0:
        return 1.0
    return float(np.sum((y_pred == rare_state) & pos) / n_pos)
