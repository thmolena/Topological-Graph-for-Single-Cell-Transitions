...
The benchmark is a seeded synthetic branching lineage (Fig.~\ref{fig:lineage}). Cells live in a
...
\caption{\textbf{The synthetic branching lineage.} Cells are plotted in the first two feature
dimensions of the embedding plane and coloured by branch (the trunk progenitor and two fates). The
deliberately rare state, carved from the late tip of one fate and nudged into its own pocket of
feature space, is highlighted as stars and comprises $\sim$\SI{4}{\percent} of cells. The figure is
regenerated deterministically from the run configuration in \texttt{summary.json}.}
\label{fig:lineage}
\end{figure}
...
error bars are 95\% confidence intervals over \num{5} seeds. All panels are drawn directly from
