"""Installed reproduction command (``stdd-reproduce``).

Runs the experiment, regenerates tables and figures, and syncs the artifacts into
the ``submission/`` folder used to build the manuscript.
"""
...
def _code_dir() -> Path:
    # code/src/stdd/reproduce.py -> parents[2] == code/
...
def _sync_submission() -> None:
    code = _code_dir()
    submission = code.parent / "submission"
    ...
    for name in ("main_results.tex", "extended_results.tex"):
...
def main(argv: list[str] | None = None) -> None:
    ...
    _sync_submission()
    print("stdd-reproduce: synced figures/tables into submission/")
